...
function draw() {
  ...
  // Apply initial scale settings for better visibility
  setTimeout(function() {
    // Adjust zoom and fit the view after network is initialized
    if (network) {
      network.fit({scale: 0.75});
  ...
  var options = {
    ...
    physics: {
      ...
      forceAtlas2Based: {
        gravitationalConstant: -100,  // More negative = stronger attraction
        centralGravity: 0.05,         // Stronger gravity to center
        springLength: 50,             // Shorter spring = closer nodes
        springConstant: 0.2,          // Stronger springs
        damping: 0.4,
        avoidOverlap: 1.0             // Maximum overlap avoidance
      },
      stabilization: {
        enabled: true,
        iterations: 300,              // More iterations for better stability
        fit: true
      }
    },
    edges: {
...
function clusterStructures() {
  ...
  // Apply better layout settings after clustering
  setTimeout(function() {
    network.fit({scale: 0.85});
    network.stabilize(200);
  }, 500);
}
...
function showOriginalUTG() {
  ...
  // Apply better layout settings after resetting
  setTimeout(function() {
    network.fit({scale: 0.85});
    network.stabilize(200);
  }, 300);
